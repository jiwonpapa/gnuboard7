...
namespace App\Upgrades\Data\Ext\Plugins\SirsoftPayNicepayments\V1_0_1\Migrations;
...
final class BackfillShopBaseRedirectUrls implements DataMigration
{
    ...
    public function run(UpgradeContext $context): void
    {
        ...
        if (! is_array($settings)) {
            $context->logger->warning('[nicepayments] 설정 JSON 형식 비정상 — 리다이렉트 주소 백필 skip');
        ...
        if ($updated === []) {
            $context->logger->info('[nicepayments] 리다이렉트 주소가 예전 기본값이 아님 — 운영자 설정 보존, 변경 없음');
        ...
        $context->logger->info('[nicepayments] 결제 리다이렉트 주소를 상점 주소 설정 기준으로 백필 완료', [
