...
/**
 * NicePayments PG 결제 취소 활동 로그 리스너.
 *
 * 코어의 sirsoft-ecommerce.payment.refund 필터 훅을 priority 20 으로 구독해
 * PaymentRefundListener (priority 10) 가 NicePayments cancel API 호출에 성공한
 * 직후 실행.
 *
...
class CancelActivityLogListener implements HookListenerInterface
{
    ...
    public function logCancelConfirmed(
        ...
        $this->logActivity('payment.cancel', [
            'loggable' => $order,
            'description_key' => 'sirsoft-pay_nicepayments::activity_log.description.payment_cancel',
            'description_params' => [
