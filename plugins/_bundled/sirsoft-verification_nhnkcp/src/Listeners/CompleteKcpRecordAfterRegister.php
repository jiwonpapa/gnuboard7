...
/**
 * 비로그인 가입 시 Cache 에 stash 된 KCP PII 를 회수하여 record 로 확정한다.
 *
 * 흐름:
 *  1. 비로그인 사용자가 KCP 본인확인 → provider.verify() 가 PII 를 Cache 에 stash
 *     (key=`nhnkcp:pending_record:{challenge_id}`, TTL=challenge.expires_at)
 ...
 *  4. 본 listener: token → log 식별 → Cache 에서 PII 회수 → record upsert
 ...
 * @since 1.0.0
 */
class CompleteKcpRecordAfterRegister implements HookListenerInterface
{
    /**
     * @param  IdentityVerificationLogRepositoryInterface  $logRepository  코어 IDV log Repository (user_id backfill)
     ...
     * @param  CacheInterface  $cache  stash 회수/정리용 (PluginCacheDriver 자동 prefix)
     */
    ...
    public function handle(...$args): void
    {
        ...
        // 코어 priority 10 listener 가 token 검증 후 consumed_at 을 set 하므로 코어 표준
        // Repository(findVerifiedForToken)는 null 을 반환한다. 본 plugin Repository 는
        // consumed_at 무관 조회를 제공한다.
        ...
        $cacheKey = KcpIdentityProvider::PENDING_RECORD_CACHE_PREFIX.$log->id;
        $piiPayload = $this->cache->get($cacheKey);

        if (! is_array($piiPayload) || $piiPayload === []) {
            Log::warning('KCP 비로그인 가입 backfill: Cache PII 부재', [
                'log_id' => $log->id,
                'user_id' => $user->id,
            ]);
        ...
        // (2) stash 된 PII 캐시는 반드시 정리한다 (TTL 만료까지 PII 잔류 방지 — PIPC).
        ...
        try {
            ...
            try {
                ...
                Log::error('KCP 비로그인 가입 backfill: log.user_id 역참조 실패', [
                    'log_id' => $log->id,
                    'user_id' => $user->id,
                    'error' => $e->getMessage(),
                ]);
            ...
            Log::error('KCP 비로그인 가입 backfill: PII record 저장 실패', [
                'log_id' => $log->id,
                'user_id' => $user->id,
                'error' => $e->getMessage(),
            ]);
