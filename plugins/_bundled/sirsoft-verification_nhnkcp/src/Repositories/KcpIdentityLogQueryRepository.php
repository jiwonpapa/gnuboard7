...
/**
 * KcpIdentityLogQueryRepositoryInterface 구현체.
 *
 * @since 1.0.0
 */
class KcpIdentityLogQueryRepository implements KcpIdentityLogQueryRepositoryInterface
{
    /**
     ...
     * 같은 토큰으로 metadata 의 식별자 해시를 회수해야 하기 때문.
     *
     ...
     *
     * user_id 를 NULL 로 비우는 동시에, 로그 metadata 에 남은 식별자 해시(di_hash / ci_hash)를
     * 함께 파기한다. CI/DI 해시는 동일인 추적에 쓰일 수 있어 탈퇴자에게도 잔존하면 안 되며
     * (PIPC 파기 의무), 감사 추적 필드(matched_field / duplicate_field_used 등)는 보존한다.
     *
    ...
    public function anonymizeUserId(int $userId): int
    {
        $logs = IdentityVerificationLog::query()
            ...
            ->where('provider_id', KcpIdentityProvider::PROVIDER_ID)
            ->get();
