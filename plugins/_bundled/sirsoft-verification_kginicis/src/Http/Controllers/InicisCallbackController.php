...
use Plugins\Sirsoft\VerificationKginicis\Http\Requests\InicisCallbackRequest;
 ...
 *
 * 검증 규칙을 두지 않는 사유: 외부 PG 가 보내는 임의 필드를 strict validation 으로 차단하면
 * 이니시스 응답을 받을 수 없다. 본 컨트롤러는 raw form POST 를 그대로 Resolver 에 전달하며,
 * 그 결정을 InicisCallbackRequest (빈 규칙) 로 명시한다.
 *
 * @since 1.0.0-beta.1
 */
class InicisCallbackController extends PublicBaseController
{
     ...
     *
     * @param  InicisCallbackRequest  $request  이니시스 form POST body
     * @return RedirectResponse
     */
    public function handle(InicisCallbackRequest $request): RedirectResponse
    {
