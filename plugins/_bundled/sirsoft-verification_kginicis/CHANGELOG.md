...
- 본인인증 팝업의 결과 전달 페이지가 주소로 전달받은 값의 형태를 먼저 확인하도록 정비했습니다. 정상 인증 흐름은 그대로이며, 주소를 임의로 조작해 들어오는 요청이 결과 페이지 안으로 흘러들지 않습니다.
...
- 본인인증 결과를 조회한 뒤 표시되는 안내 문구가 번역된 문장 대신 내부 식별자가 그대로 보이던 문제를 수정했습니다. 이제 사용 중인 언어에 맞는 안내 문구가 표시됩니다.
