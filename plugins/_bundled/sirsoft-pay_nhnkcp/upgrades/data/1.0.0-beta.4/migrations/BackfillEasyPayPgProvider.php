...
namespace App\Upgrades\Data\Ext\Plugins\SirsoftPayNhnkcp\V1_0_0_beta_4\Migrations;
...
/**
 * 저장된 이커머스 주문설정의 NHN KCP 간편결제 항목에 PG 고정 선언을 백필한다.
 *
 * 배경 (이슈 #475):
 *
...
final class BackfillEasyPayPgProvider implements DataMigration
{
     ...
     */
    private const METHOD_PREFIX = 'nhnkcp_';
    ...
    public function run(UpgradeContext $context): void
    {
        ...
        if (! is_array($settings) || ! is_array($settings['payment_methods'] ?? null)) {
            $context->logger->warning('[nhnkcp] 주문설정 JSON 형식 비정상 — 간편결제 PG 백필 skip');
        ...
        if ($updated === 0) {
            $context->logger->info('[nhnkcp] 간편결제 PG 선언이 이미 최신 — 변경 없음');
        ...
        $context->logger->info('[nhnkcp] 간편결제 PG 고정 선언 백필 완료', [
