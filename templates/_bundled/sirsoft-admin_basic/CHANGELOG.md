...
- 환경설정 > 일반 에 「초기 화면 정적 파일」 카드가 추가되었습니다. 게시 상태(게시됨·미게시·비활성·개발 모드)와 현재 버전·파일 수·마지막 게시 시각·실행 계정·최근 실패 사유를 보여 주고, 「지금 다시 만들기」로 확인 후 즉시 다시 만들 수 있습니다. 게시가 쓰이지 않는 환경에서는 버튼이 비활성으로 표시되고 그 이유를 안내합니다.
- 대시보드의 「초기 화면 파일 생성 실패」 알림에 「다시 만들기」 버튼이 붙어, 알림에서 바로 다시 만들 수 있습니다.

- 아이콘(Font Awesome), 본문 글꼴(Pretendard), 국기 아이콘, 레이아웃 코드 편집기(Monaco)를 템플릿에 함께 담았습니다. 관리자 화면 전체가 외부 CDN 없이 동작합니다.
...
- 레이아웃 코드 편집기(Monaco)를 0.56.0 으로 올렸습니다. 편집기가 자체적으로 담고 있던 HTML 정화 라이브러리가 크게 뒤처져 있어 함께 개선됩니다. (#126 @jiwonpapa 님께서 제보해주셨습니다.)
