...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('plugins', function (Blueprint $table) {
            $table->id();
            $table->string('identifier')->unique()->comment('플러그인 고유 식별자 (vendor-plugin 형식)');
            $table->string('vendor')->index()->comment('벤더/개발자명');
            $table->text('name')->comment('플러그인 이름 (다국어 JSON)');
            $table->mediumText('description')->nullable()->comment('플러그인 설명 (다국어 JSON)');
            ...
            $table->string('version')->comment('플러그인 버전');
