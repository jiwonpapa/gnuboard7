...
- 통합검색에서 Manticore를 선택하면 상품 검색과 카테고리 필터를 검색 전용 인덱스에서 처리하고, 현재 페이지의 상품만 데이터베이스에서 불러오도록 개선했습니다.
...
- 통합검색의 상품 전체 건수와 현재 페이지를 window count 한 번으로 조회해 같은 FULLTEXT 검색을 두 번 실행하지 않도록 개선했습니다.
