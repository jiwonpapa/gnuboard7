 ...
 * `sirsoft-ecommerce.*` 등 임의 string. 시드는 모듈 자체 artisan 커맨드(playwright:seed-ecommerce) 호출.
 */
...
function resolveBaseUrl(): string {
  ...
  throw new Error(
    '이커머스 모듈 E2E base URL 미설정. PLAYWRIGHT_BASE_URL 환경변수를 지정하거나 코어 .env 의 APP_URL 을 활성 호스트로 설정하세요.'
  );
}
