...
return new class extends Migration
{
     ...
     * Run the migrations.
     *
     * board_posts.trigger_type enum 에 'cascade' 값을 추가합니다.
     * 원글 삭제 시 함께 지워지는 답글 트리를 'cascade' 로 마킹하여, 원글 복원 시
     * 사용자가 직접 지운 답글('user')과 구분해 선별 복원하기 위함입니다
     * (댓글 2026_06_26_000001 / 첨부 2026_06_26_000002 와 동일 규약).
     */
    public function up(): void
    {
        $prefix = DB::getTablePrefix();

        // enum 에 'cascade' 추가 (조치 주체: 원글 삭제 연쇄)
        DB::statement(
            "ALTER TABLE {$prefix}board_posts MODIFY COLUMN trigger_type "
            ."ENUM('report', 'admin', 'system', 'auto_hide', 'user', 'cascade') "
            ."NOT NULL DEFAULT 'admin' "
            ."COMMENT '조치 주체 (report: 신고, admin: 관리자, system: 시스템, auto_hide: 자동 블라인드, user: 사용자 직접 삭제, cascade: 원글 삭제 연쇄)'"
        );
    }
