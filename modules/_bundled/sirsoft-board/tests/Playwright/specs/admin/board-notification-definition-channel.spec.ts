/**
 * 게시판 알림 정의 탭 채널 계약 — 목록 요청이 화면 채널을 나르는지 (#518 / 공개 #76).
 *
 * 결함(2026-08-05 브라우저 실측): 목록 프루닝으로 서버가 `template_channel` 을 받은 요청에만
 *   `templates` 를 싣게 됐는데, 코어 설정 화면만 그 파라미터를 보내도록 고쳐지고 게시판 탭
 *   데이터소스는 빠졌다. 7건 전 행이 「이 채널에 대한 템플릿이 없습니다」로 표시되고
 ...
 *   행 수도 정상이었다.
 *
 * 시드 의존: 게시판 모듈 알림 정의(최소 1건). 커스터마이즈 축은 화면에서 만들고 되돌린다.
 */
import { test, expect, authenticatePage } from '../../fixtures/board-auth';
import type { Page } from '@playwright/test';

const SETTINGS_URL = '/admin/boards/settings?tab=notification_definitions';
...
async function assertChannelIsCarried(page: Page, channel: string) {
    ...
    test.skip(rows.length === 0, '게시판 알림 정의 시드가 없어 판정할 수 없다');
...
async function assertResetButtonSpansChannels(page: Page, channel: string) {
    ...
    test.skip((payload?.data?.data ?? []).length === 0, '게시판 알림 정의 시드가 없어 판정할 수 없다');
...
test.describe('게시판 알림 정의 탭 — 채널 계약 (#518)', () => {
    /**
     * @scenario channel=mail,template_state=default
     * @effects board_notification_list_requests_template_channel, board_notification_channel_reads_single_source
     */
    ...
    test.describe('커스터마이즈 상태', () => {
         ...
         * @effects board_notification_reset_button_uses_server_aggregate
         */
