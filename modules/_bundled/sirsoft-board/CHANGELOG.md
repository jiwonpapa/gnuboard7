...
- 게시판을 ID순으로 정렬할 때 다른 게시판의 대량 행을 건너뛰지 않고 게시판별 목록 인덱스를 사용하도록 개선했습니다.
- 통합검색에서 Manticore를 선택한 경우 공개 게시글의 현재 페이지와 정확한 전체 건수를 검색 전용 인덱스에서 조회하고, 연결 장애 시 기존 MySQL 검색으로 자동 복귀하도록 개선했습니다.
- 대용량 FULLTEXT 검색이 메모리 보호 상한에 걸릴 때 일반 오류 대신 최근 게시글 제한 범위에서 안전하게 검색하도록 개선했습니다.
- 회원명 검색 후보를 먼저 제한해 MySQL의 `IN` 하위 쿼리 `LIMIT` 비지원 오류를 방지했습니다.
- 제한 검색의 최근 게시글 선별이 게시판별 ID 인덱스를 사용하도록 변경해 게시판 생성 순서에 따른 검색 지연을 방지했습니다.

## [1.1.3] - 2026-07-16

### Fixed

- 대용량 게시판 검색이 전체 결과를 무제한 집계해 서버 자원을 고갈시키던 문제를 제한형 후보 조회, 동시 실행 차단, 검색 요청 제한으로 개선했습니다.
...
- 대용량 목록 최적화와 그누보드7 7.0.4 원본 조회 경로를 즉시 전환해 동일 서버에서 반복 비교할 수 있는 benchmark variant를 추가했습니다.

## [1.1.0] - 2026-07-15

### Changed

- 게시글 목록은 정렬 인덱스에서 현재 페이지의 ID만 먼저 찾고 선택된 게시글만 본문 요약·작성자·썸네일과 결합하도록 변경해, 대용량 게시판의 깊은 페이지에서 넓은 행을 대량 스캔하지 않습니다.
- ID순·조회수순 목록 전용 복합 인덱스를 추가하고, 검색·분류 등 필터 목록의 전체 건수는 동일 조건을 60초간 재사용하도록 개선했습니다.
- 첫 페이지 공지는 최대 10건, 한 페이지에 펼치는 답글은 최대 100건으로 제한하고 답글이 없는 페이지에서는 답글 조회를 생략합니다.
