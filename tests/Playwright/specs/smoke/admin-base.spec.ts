...
test('@smoke #399 Phase 1.3 - admin_settings 진입 시 sticky-tab-nav 시맨틱과 TabNavigation 시맨틱 탭 버튼이 적용된다', async ({ page }) => {
  ...
  //
  // TabNavigation 은 자신이 <nav> 를 렌더하고 id 를 그 위에 얹는다(Nav id={id}).
  // 따라서 #tab_navigation 이 곧 nav.tab-nav-default 이며, 그 안에 또 nav 가 있지 않다.
  const nav = stickyNav;
  await expect(nav).toHaveClass(/tab-nav-default/);
...
test('@smoke #399 Phase 1.4 - admin_board_settings 진입 시 sticky_header 가 sticky-section-header 시맨틱으로 마운트된다', async ({ page }) => {
  // admin_board_settings.json 의 permissions 는 `sirsoft-board.settings.read` 다.
  // (`...settings.view` 는 존재하지 않는 권한 키라 403 Access Denied 로 떨어진다)
  const token = issueToken('sirsoft-board.settings.read');
  await authenticatePage(page, token);

  // 게시판 설정의 실제 라우트는 복수형 `/admin/boards/settings` 다.
  // 단수형 `/admin/board/settings` 는 `*/admin/board/:slug`(게시글 목록) 에 매칭되어
  // slug="settings" 인 존재하지 않는 게시판을 조회 → 403 으로 떨어진다.
  await page.goto('/admin/boards/settings');
  await page.waitForLoadState('domcontentloaded', { timeout: 30_000 });
  await page.waitForFunction(
    () => !window.location.pathname.includes('/admin/login') || document.readyState === 'complete',
    { timeout: 15_000 },
...
test('@smoke #399 Phase 1.13 - admin_identity_logs 의 filter_section (필터-그리드 바깥 filter-row 부모) 도 .filter-grid 시맨틱 적용', async ({ page }) => {
  ...
  //
  // 단, #advanced_filters 는 `if: _local.filter.advancedOpen === true` 로 접혀 있는
  // 조건부 노드다. 고급 검색을 펼치지 않으면 DOM 에 존재하지 않으므로, 실제 사용자
  // 흐름대로 토글을 먼저 연 뒤 시맨틱을 검증한다.
  await page.locator('#advanced_filters_toggle').click();

  await expect(page.locator('#advanced_filters')).toBeVisible({ timeout: 10_000 });
