...
- 공개 통합검색의 게시글·상품·페이지 조회를 로컬 Manticore로 전환할 수 있도록 했습니다. 일반 게시판·관리자 검색은 기존 MySQL 경로를 유지하며, 검색 데몬에 장애가 생기면 MySQL로 자동 복귀합니다.
- 공통·게시판·쇼핑몰 성능 개선을 영역별 또는 전체로 즉시 켜고 끌 수 있는 통합 하네스를 추가했습니다. 실행 코드·인덱스·활성 확장 복제본을 함께 검증하며, 미커밋 파일은 배포하지 않고 혼합·변조 상태를 실패로 판정합니다.
- 요청 부팅 중 훅마다 남기던 성공 로그를 리스너별 요약으로 줄이고, 비회원 권한·활성 모듈·언어팩 재사용을 게시판 전용 설정에서 공통 성능 설정으로 분리했습니다.
- 게시판과 상품의 통합검색은 전체 매칭 ID를 PHP로 옮기거나 FULLTEXT와 작성자 조건을 한 OR로 묶지 않도록 개선하고, 쇼핑몰 storefront와 상품 목록은 같은 요청의 분류 트리·권한·가격 변환 결과를 재사용하도록 변경했습니다.
- 게시판·상품 통합검색의 전체 건수와 현재 페이지를 한 번의 조회로 계산하고, 게시판 작성자 부분검색은 작은 고유 작성자 사전을 사용하도록 개선해 같은 대용량 검색을 반복 실행하거나 게시글 전체를 검사하지 않게 했습니다.
