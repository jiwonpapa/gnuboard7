...
- 기반 프레임워크를 Laravel 13으로 올리고 최소 실행 환경을 PHP 8.3으로 상향했습니다.
- Laravel 13에서 이름이 바뀐 CSRF 미들웨어를 결제·본인인증 콜백 라우트에 반영했습니다. 영향받는 번들 플러그인의 패치 버전을 함께 올렸으며, 공식 G7 코어 최소 버전은 변경하지 않았습니다.
- 기존 로그인 세션과 G7의 객체 캐시 계약을 보존하도록 Laravel 13 직렬화 설정을 명시했습니다.
- Laravel 13 콘솔 명령 정의 방식에 맞춰 `db:seed`의 G7 전용 `--count`·`--sample` 옵션을 시그니처에 등록했습니다.

### Performance

- Performance Lab: 확장 설치·업데이트의 기본 완료 경로에서 MySQL FULLTEXT 자기 매칭 점검을 분리했습니다. 대용량 게시판에서는 읽기 점검만으로도 FULLTEXT 초기화와 메모리·swap 급증이 발생하므로, 인덱스 점검·복구는 `search:index` 또는 `--rebuild-search-index`를 명시했을 때만 수행합니다. 공개 API 변경이 아닌 콘솔 내부 실행 정책 변경이므로 번들 확장의 최소 코어 버전은 추가 상향하지 않습니다.

### Added

- Laravel Octane을 선택적으로 사용하는 환경에서 모듈·플러그인·템플릿·주요 설정을 변경하면, 현재 작업이 끝난 뒤 실행 중인 워커가 새 코드와 설정을 자동으로 다시 읽도록 지원합니다. Octane을 사용하지 않는 기존 환경에는 영향을 주지 않습니다.
- 기존 PHP 실행 방식과 임시 Octane 환경을 같은 기능·부하 조건으로 비교하고, 측정 후 의존성·설정·프로세스를 원래 상태로 복구하는 점검 도구를 추가했습니다.

### Fixed

- 장시간 실행되는 PHP 환경에서 요청별 권한·훅 상태와 게시판 응답값이 다음 요청에 남을 수 있는 부분을 초기화하도록 보완했습니다.
- PHP 8.5에서 화면을 불러올 때 확장 캐시 처리 관련 경고가 반복될 수 있던 문제를 수정했습니다.
- Laravel 메이저 버전 업데이트 중 구버전 DB 연결 객체와 신버전 마이그레이터가 한 프로세스에 섞이지 않도록, 파일 교체 후 마이그레이션을 새 PHP 프로세스에서 실행합니다.
