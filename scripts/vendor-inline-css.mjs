 ...
 *
 * 이 사본은 **코어 소유**다 — 설치 마법사(`public/install/assets/vendor/`)의 자산을 만든다.
 * 확장은 각자 `{확장}/scripts/vendor-inline-css.mjs` 를 소유한다. 사본이 여럿인 것은
 * 중복이 아니라 소유권 분리다 — 코어의 재생성이 특정 템플릿의 존재에 의존하면 안 되고,
 * 제3자 템플릿 저작자도 코어 없이 자기 자산을 다시 만들 수 있어야 한다. 동봉 FA 버전도
 * 서로 다르다(설치 마법사 6.5.1 / 템플릿 6.4.0).
 *
 * 생성기는 `public/` 밖에 둔다. 게시 디렉토리 안에 두면 배포된 모든 사이트에서
 * HTTP 로 열려 내부 판단 근거(브라우저 하한·저장소 조사 결과)가 그대로 노출된다.
 *
 * 설치 마법사 자산 재생성:
 *   node scripts/vendor-inline-css.mjs  *     <FA 6.5.1 패키지>/css/all.min.css  *     public/install/assets/vendor/font-awesome/6.5.1/webfonts  *     public/install/assets/vendor/font-awesome/6.5.1/css/all.inlined.css
 */
