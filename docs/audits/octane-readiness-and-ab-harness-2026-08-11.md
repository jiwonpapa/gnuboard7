...
| 공통 조건 | 값 |
|---|---|
| 실서버 | `g7devops.com`, Linux 공유 VPS, **2 vCPU·2GB** |
| 공통 웹 앞단 | Nginx·기존 HTTPS |
| 애플리케이션 | Laravel 12.62.0·동일 운영 소스·DB |
| 데이터 | 자유게시판 50,002건·댓글 62,853건 |
| 부하 | 각 경로 5 VU·15초, 최대 6 worker |

| 비교 구성 | 요청 경로 | PHP | worker |
|---|---|---:|---:|
| PHP-FPM | Nginx → PHP-FPM | 8.5.8 | 동적, 최대 6 |
| 초기 `Octane` | Nginx → Octane → RoadRunner | 8.5.8 | 고정 6 |
| FrankenPHP 시험 | Nginx → Octane → FrankenPHP | 내장 8.5.9 | 고정 6 |

### 3개 실행 구성 성능 비교

![PHP-FPM·Octane RoadRunner·Octane FrankenPHP 실서버 성능 비교](assets/octane-runtime-comparison-2026-08-11.png)

| PHP-FPM 대비 5경로 단순 평균 | Octane + RoadRunner | Octane + FrankenPHP |
|---|---:|---:|
| 평균 응답시간 | **40.7% 감소** | **46.8% 감소** |
| p95 응답시간 | **39.6% 감소** | **45.0% 감소** |
| 처리량 | **77.3% 증가** | **96.8% 증가** |

한눈에 보면 두 Octane 구성 모두 PHP-FPM보다 빨랐습니다. FrankenPHP는 5경로 중 4경로에서 평균 응답과 처리량이 앞섰지만, **쇼핑몰 p95는 RoadRunner보다 53.6% 느렸습니다.** 내장 PHP·Caddy·DB socket 관리도 추가되므로 현재 결과만으로 FrankenPHP로 바꿀 근거는 부족합니다.

<details>
<summary><strong>경로별 원시 수치·개선율 보기</strong></summary>

각 수치는 `평균 응답 ms / p95 ms / 처리량 req/s`입니다.

| 경로 | PHP-FPM | Octane + RoadRunner | PHP-FPM 대비 개선<br>(평균 / p95 / 처리량) | Octane + FrankenPHP | PHP-FPM 대비 개선<br>(평균 / p95 / 처리량) |
|---|---:|---:|---:|---:|---:|
| 메인 `/` | 199.24 / 248.89 / 24.90 | 99.20 / 124.86 / 50.03 | **50.2% / 49.8% / +100.9%** | 87.85 / 106.16 / 56.55 | **55.9% / 57.3% / +127.1%** |
| 게시판 화면 `/board/freebd` | 209.55 / 323.27 / 23.67 | 120.64 / 208.65 / 41.20 | **42.4% / 35.5% / +74.1%** | 92.00 / 138.24 / 53.97 | **56.1% / 57.2% / +128.0%** |
| 게시판 API 첫 페이지 | 226.83 / 275.41 / 21.91 | 152.53 / 203.21 / 32.60 | **32.8% / 26.2% / +48.8%** | 132.92 / 166.48 / 37.42 | **41.4% / 39.6% / +70.8%** |
| 쇼핑몰 화면 `/shop/products` | 247.40 / 523.24 / 20.06 | 104.35 / 155.71 / 47.61 | **57.8% / 70.2% / +137.3%** | 110.76 / 239.16 / 44.89 | **55.2% / 54.3% / +123.7%** |
| 상품 API 첫 페이지 | 400.13 / 516.48 / 12.44 | 319.12 / 433.00 / 15.57 | **20.2% / 16.2% / +25.2%** | 297.65 / 431.85 / 16.69 | **25.6% / 16.4% / +34.2%** |

</details>

이 단순 평균은 경로별 실제 트래픽 비중을 반영하지 않은 참고치입니다. 측정 시각은 PHP-FPM·RoadRunner 2026-08-11 15:27~15:37 KST, FrankenPHP 16:10~16:13 KST입니다.
