...
그누보드7은 **RoadRunner 기반 Laravel Octane을 선택 기능으로 지원할 수 있지만, 아직 PHP-FPM을 기본값으로 대체하면 안 됩니다.** 자유게시판 50,002건 상태에서 `g7devops.com`의 공개 HTTPS 경로를 실제 전환하고 PHP-FPM·Octane 모두 최대 6 worker 조건으로 다시 비교한 결과, 다섯 경로 모두 평균 응답시간은 20.2~57.8%, p95는 16.2~70.2% 감소했습니다. 처리량은 25.2~137.3% 증가했습니다.

이전 2-worker 시험에서 나타난 게시판 첫 페이지 API의 p95 12.2% 악화는 6-worker 재시험에서 **26.2% 개선**으로 바뀌었습니다. 따라서 이전 악화의 주원인은 Octane 자체보다 `5 VU : 2 worker` 대기열로 판단합니다. 다만 2 vCPU·2GB 서버에서 6 worker는 약 531MB를 사용했고 swap 입출력도 관측됐습니다. 일부 경로의 p99·최대 응답에는 단발성 지연도 남아 있어 장시간 안정성 시험 전에는 기본 전환하면 안 됩니다.

시험이 끝난 뒤 PHP-FPM으로 복구했으며 Nginx·Composer 체크섬, 공개 응답, 서비스, 실행 프로세스와 포트를 확인했습니다. 운영 서버에 Octane은 남아 있지 않습니다. 판정은 `선택 설치 가능`, `기본 전환 보류`입니다.
...
## 공개 실서버 6-worker·5 VU 비교

- 측정 시각: 2026-08-11 15:27~15:37 KST
- 서버: `g7devops.com`, Linux, PHP 8.5.8, Laravel 12.62.0
- 비교: 동일 서버·도메인·TLS·Nginx·DB에서 현재 PHP-FPM과 RoadRunner Octane
- 실행본: 운영 소스와 Laravel 12.62.0은 동일하며, Octane 관련 패키지만 별도 임시 실행본에 설치
- 데이터: 자유게시판 50,002건, 댓글 62,853건
- PHP-FPM: 동적 프로세스, `pm.max_children=6`
- Octane: 고정 6 worker, `max-requests=500`
...
`평균 개선`과 `p95 개선`은 `(PHP-FPM - Octane) / PHP-FPM`, `처리량 증가`는 `(Octane / PHP-FPM) - 1`로 계산했습니다. 다섯 경로의 단순 평균은 평균 응답시간 40.7% 감소, p95 39.6% 감소, 처리량 77.3% 증가입니다. 서로 다른 경로의 단순 평균이므로 실제 트래픽 비중을 반영한 종합 점수는 아닙니다.
...
다만 worker 수만 같을 뿐 메모리 사용 방식까지 같지는 않습니다. PHP-FPM은 요청량에 따라 자식 프로세스를 늘리지만 Octane은 6개를 계속 유지합니다. Octane 시작 후 실측 RSS는 약 531MB였고, 부하 중 가용 메모리는 약 300~345MB였습니다. `vmstat`에서는 최저 여유 메모리 약 86MB와 swap 입출력이 관측됐습니다. 성능은 좋아졌지만 이 2GB 서버에서 6 worker를 운영 기본값으로 확정할 근거는 아닙니다.

또한 게시판 화면의 p99는 396.27→569.45ms, 최대값은 451.68→894.55ms로 악화됐고 상품 API 최대값도 632.74→981.09ms로 증가했습니다. p95까지는 모두 개선됐지만 짧은 15초 시험에서 소수의 큰 지연이 있었으므로 장시간 반복 시험이 필요합니다.

### 공개 복구 중 확인된 문제

첫 2-worker 시험에서는 공개 경로를 PHP-FPM으로 먼저 되돌린 뒤 Composer가 `vendor`를 원복하는 짧은 구간에 공개 요청 1건이 실패했습니다. 이번 6-worker 시험은 라이브 `vendor`를 수정하지 않는 별도 임시 실행본을 사용하고, `PHP-FPM 기동·확인 → Nginx 전환 → 기존 연결 종료 대기 → Octane 종료` 순서로 복구했습니다. 부하·복구 구간의 신규 PHP·Nginx 오류는 0건이었고 공개 오류도 없었습니다.
...
| 재적용 후 공개 요청 | 200 |
| 6-worker 부하·복구 중 신규 PHP·Nginx 오류 | 0 |
...
- 별도 실행본 전환 방식을 정식 하네스에 편입하고 중단 상황에서도 자동 복구되는지 검증
