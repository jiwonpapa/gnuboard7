# [제안] Laravel 13 코어 전환이 필요합니다

## 결론

그누보드7 코어의 Laravel 13 전환을 공식 작업으로 진행해야 합니다. Laravel 12는 이미 일반 버그 수정 지원이 끝났고, 보안 업데이트도 **2027년 2월 24일** 종료됩니다. 보안 지원 종료가 임박한 상태에서 개별 운영 사이트가 코어를 임의 수정하게 두기보다, 코어 8.0.0 범위에서 호환성과 업데이트 절차를 함께 검증하는 것이 안전합니다.
...
- G7 7.0.9 → 8.0.0 코어 업데이트·롤백을 실제 설치 환경에서 검증
- 페이지빌더 등 활성 커스텀 확장이 보존되고 주요 공개 화면·API가 정상 응답

## 제안 일정

보안 지원 종료 직전의 긴급 전환을 피하도록 **2026년 안에 코어 8.0.0 전환과 배포 검증을 완료**하는 것을 제안합니다.
