...
## [engine-v1.68.2] - 2026-09-08

### Fixed

- 일반 페이지 JSON의 빈 객체·배열·숫자 키 객체를 검증·출처 마스킹 후 저장, 편집 재열기, 이력 조회·복원에서 보존한다. 기존 배열 기반 내부 렌더와 검증 규칙은 유지한다.
- 같은 레이아웃의 버전 확인·본문·이력 저장을 행 잠금 트랜잭션으로 처리한다. 이력 복원도 잠금 버전을 증가시켜 복원 이전 편집본의 덮어쓰기를 거부한다.
- 편집기 전역 단축키가 확장 버튼·링크·접이식/선택 컨트롤의 기본 키 동작을 가로채지 않도록 수정한다.

NE6 호스트 후보. G7 상위 저장소 반영 여부와 운영 적용 파일 지문은 PB 릴리스의 NAT-06 기록에서 별도로 확인한다.
...
- 공개 확장 호스트에서 선택 조합을 내보내고 허용 위치에 다시 삽입할 수 있다. 템플릿·컴포넌트 설정·첨부 파일과 현재 선택을 확인하고, 새 ID와 내부 참조를 연결해 한 번의 Undo/Redo로 처리한다.
- 데이터 바인딩과 외부 노드 참조가 있는 조합은 원래 페이지 안에서만 재사용하며 보호된 출처·지원하지 않는 구조·오래된 요청을 거부한다. 저장소와 사용자 목록은 확장이 소유하고 기존 페이지 저장 경로를 유지한다.

선택형 compositions 계약을 제공하는 로컬 NE4 후보다. 기존 확장은 변경 없이 동작하며 G7 배포/운영 제공을 뜻하지 않는다.
...
- 일반 페이지 확장 패널을 `G7Core.layoutEditor.registerPanel`로 등록하고 해제할 수 있다. 선택 항목의 불변 문맥과 명령은 기존 문서·Undo/Redo를 공유한다. 오래된 세션·변경 시점·노드·잠금 상태의 요청은 변경 없이 거부한다.
- 확장 명령 v1은 route 소유 평문 수정과 nesting 규칙이 허용하는 basic 자식 삽입을 제공한다. 다른 편집 모드와 바인딩은 지원으로 오인하지 않도록 거부한다. 기존 확장의 props에 선택형 `extensionHost`를 추가한다.

### Fixed

- 공개 캔버스 삽입 콜백의 문자열 경로를 실제 ComponentPath로 검증·변환하고 반응형 경로를 보존한다. 잘못된 문자열은 루트 삽입으로 처리하지 않는다.
- 일반 페이지 전환·재로드 이후 도착한 이전 문서 응답을 반영하지 않는다. 동기 문서 셀을 통해 연속 변경과 이력의 스냅샷 순서를 일치시킨다.

기존 4개 등록 메서드의 호출 방식은 유지한다. 번들 확장은 새 패널 API를 사용하지 않아 최소 G7 버전 변경 대상이 없다. 새 API 소비자는 기능 감지와 프로토콜 `g7.layout-editor/1` 검증이 필요하다.
